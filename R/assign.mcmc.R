assign.mcmc <- function(Y, Bg, X, Delta_prior_p, iter=2000, adaptive_B=TRUE, adaptive_S=TRUE, mixture_beta=TRUE, sigma_sZero = 0.001, sigma_sNonZero = .001, p_beta = 0.01, sigma_bZero = 0.01, sigma_bNonZero = 1, alpha_tau = 1, beta_tau = 0.01, Bg_zeroPrior=TRUE, S_zeroPrior=FALSE, ECM = FALSE)
{  
  ...
  for (i in 2:iter) {
    ...
    if (adaptive_S == TRUE){
      ...
      for (j in 1:m){
        ...
        S_temp[,j] <- mu_S_1 #rnorm(n, mu_S_1, sqrt(s_S_inv_1[, j]))
      ...
      S_temp <- ifelse(delta_temp==1, S_0, 0)
    }
